...
int speedVal;                     // Stepper motor speed, controlled by pot on A0
int ampVal;                       // Stepper motor  amplitude, controlled by pot on A1
int shakerState = 0;
...
void loop() {
  checkButtonState();               // Check 2 Buttons state
	if (state == HIGH) {
		for (int i = 0; i<2; i++) {
			if (i == 0) { 
